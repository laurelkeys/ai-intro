...
class Runner:
    def __init__(self, image_path, polygon_count, vertices_count, population_size=1,
        ...
        # NOTE self.pil_image is a PIL image, self.image is its Numpy array representation
        if max(self.pil_image.size) > max(max_internal_size):
            self.image = np.array(self.pil_image.copy().thumbnail(max_internal_size), dtype=np.uint8) # keeps proportions
        else:
            self.image = np.array(self.pil_image, dtype=np.uint8)
    ...
    def run(self, use_partial_fitness=True, use_image_colors=True):
        ...
        population = Population(
            ...
            original_image=self.pil_image if use_image_colors else None # uses image's colors on starting polygons
        )
